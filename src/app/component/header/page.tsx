...
import { SuccessfullLogin } from '@/app/components/Auth/AuthDialog/SuccessfulLogin'
import { FailedLogin } from '@/app/components/Auth/AuthDialog/FailedLogin'
import { UserRegistered } from '@/app/components/Auth/AuthDialog/UserRegistered'
...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    // route change हुँदा mobile sidebar auto close
    setNavbarOpen(false)
  }, [path])
  ...
  return (
    <header
      className={`fixed h-24 top-0 py-1 z-50 w-full dark:bg-transparent transition-all border-b-2 border-gray-200 dark:border-gray-600 ${sticky
        ? 'shadow-lg bg-white dark:shadow-dark-md dark:bg-darklight!'
        : 'shadow-none'
        }`}>
      <div className='container mx-auto max-w-6xl px-4 flex items-center justify-between p-6'>
        <Headerlogo />
    ...
    rounded-full
    bg-white dark:bg-darklight dark:text-white
    shadow-md
  ...
  "
          >
            ...
            <svg
              viewBox="0 0 16 16"
              className={`hidden h-6 w-6 dark:block transition-colors duration-300 ${!sticky && pathUrl === '/' ? 'text-white' : ''
                }`}
            >
              <path
            ...
            <svg
              viewBox="0 0 23 23"
              className={`h-6 w-6 dark:hidden transition-colors duration-300 text-white`}
            >
          ...
          <Link
            href='#'
            className='hidden lg:block bg-transparent border border-success text-success px-4 py-2 rounded-lg hover:bg-success dark:border-primary dark:hover:bg-primary dark:text-blue-600 dark:hover:text-white  hover:text-white'
            onClick={() => {
          ...
          <button
            ...
            className="
    block lg:hidden p-2 rounded-lg
    bg-white dark:bg-darklight
    shadow-md hover:shadow-lg focus:shadow-outline
    transition-all duration-300 ease-in-out
  ...
  "
          ...
          >
            <span className="block w-6 h-0.5 bg-black dark:bg-white transform transition-all duration-300"></span>
            <span className="block w-6 h-0.5 bg-black dark:bg-white mt-1.5 transform transition-all duration-300"></span>
            <span className="block w-6 h-0.5 bg-black dark:bg-white mt-1.5 transform transition-all duration-300"></span>
          </button>
  ...
  <button
    ...
    className="
      p-2
      rounded-full
      hover:bg-gray-200 dark:hover:bg-gray-700 dark:bg-darklight
      focus:outline-none 
